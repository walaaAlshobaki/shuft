...
class _TrainerRegistrationState extends State<TrainerRegistration> {
  ...
  File fileCar;
  File fileDriverLicensimage;
  void _choose() async {
    //  file = await ImagePicker.pickImage(source: ImageSource.camera);
    fileCar = await ImagePicker.pickImage(source: ImageSource.gallery);
    if (fileCar == null) return;
    base64Image = base64Encode(fileCar.readAsBytesSync());
    fileName = fileCar.path.split("/").last;
  }

  void _chooseDriverLicensimage() async {
    //  file = await ImagePicker.pickImage(source: ImageSource.camera);
    fileDriverLicensimage =
        await ImagePicker.pickImage(source: ImageSource.gallery);
    if (fileDriverLicensimage == null) return;
    base64ImageDriverLicensimage =
        base64Encode(fileDriverLicensimage.readAsBytesSync());
    fileNameDriverLicensimage = fileDriverLicensimage.path.split("/").last;
  }
  ...
  Future userLogin() async {
    ...
    final trainairDataModel = TrainairDataModel(
      firstName: firstname,
      lastName: lastname,
      email: email,
      password: password,
      ...
      IDNum: id,
      Gender: gende,
      driverLicens: fileDriverLicensimage,
      carImage: fileCar,
      testImage: fileCar,
      shiftGear: carType,
      details: details,
    ...
    _apiResponse.registerTrainaier(trainairDataModel, context);
  }
